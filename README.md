...
- MiniMessage output is always a single compact `<gradient:#aaa:#bbb:...>text</gradient>` tag built from the original color stops. Adventure's own gradient tag always interpolates in plain RGB, so in HSL/OKLab mode the in-game look is a close approximation of the tool's own preview rather than an exact match; every other output format (legacy, shorthex, JSON, BBCode) still emits an exact per-character color and matches the preview precisely.
...
- Ported from birdflop/web's `AnimTABUtils.generateAnimTABFrames`, simplified the same way the static gradient generator is: no rich per-selection formatting and no custom (non-evenly-spaced) color-stop positions. MiniMessage output uses birdflop's own trick for animated frames: each of the original color stops gets its own shifted copy of the gradient (`shiftedGradientStops()`), and the frame is one `<gradient:c1:c2:...>` tag built from those — so Adventure animates the scan itself instead of needing an explicit color before every character. Full-text-cycle frames use a plain `<color:#hex>` tag instead, since every character already shares one color there. Note that Adventure's `<gradient>` tag always interpolates the pixels *between* stops in plain RGB, so in HSL/OKLab mode the in-game look is a close approximation of the tool's own preview rather than an exact match — every other output format (legacy, shorthex, JSON, BBCode) still emits an exact per-character color and matches the preview precisely.
